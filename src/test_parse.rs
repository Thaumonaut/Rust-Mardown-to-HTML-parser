...
fn parse_heading(pair: pest::iterators::Pair<Rule>) -> String {
  ...
  html_element_builder(tag, content.as_str())
}
...
fn html_element_builder(tag: String, content: &str) -> String {
    format!("<{t}>{c}</{t}>\n", t = tag, c = content)
}
    ...
    for inner_pair in pair.into_inner() {
        match inner_pair.as_rule() {
                ...
                if let Some(content_pair) = inner.next() {
                    match content_pair.as_rule() {
                        Rule::italic => {
                            ...
                            content.push_str(&html_element_builder("strong".to_string(), &html_element_builder("em".to_string(), italic_content)));
                        },
                        _ => {
                            content.push_str(&html_element_builder("strong".to_string(), &html_element_builder("em".to_string(), content_pair.as_str())));
                        }
                    }
                }
            },
            Rule::bold => {
                    ...
                    for item in bold_content_pair.into_inner() {
                        match item.as_rule() {
                            Rule::italic => {
                                ...
                                bold_html.push_str(&html_element_builder("em".to_string(), italic_content));
                            },
                            Rule::bold_text => {
                ...
                content.push_str(&html_element_builder("strong".to_string(), &bold_html));
            },
            Rule::italic => {
                let inner_content = inner_pair.into_inner().nth(1).unwrap().as_str();
                content.push_str(&html_element_builder("em".to_string(), inner_content));
            },
            Rule::inline_code => {
                let inner_content = inner_pair.into_inner().nth(1).unwrap().as_str();
                content.push_str(&html_element_builder("code".to_string(), inner_content));
            },
            Rule::strikethrough => {
                let inner_content = inner_pair.into_inner().nth(1).unwrap().as_str();
                content.push_str(&html_element_builder("s".to_string(), inner_content));
            },
            Rule::text => {
                content.push_str(inner_pair.as_str());
            },
